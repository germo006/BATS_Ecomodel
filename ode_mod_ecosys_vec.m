...
Pmax_phy = po_vals(40) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
growPHYc = y_vals(19) * Pmax_phy *...
    (1 - exp(-po_vals(84) * (y_vals(18)/y_vals(19)) * PAR / Pmax_phy)) *...
    exp(-po_vals(80) * PAR);
...
Pmax_TR = po_vals(38) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
if Pmax_TR == 0
    growTRc = 0;
else
    growTRc = y_vals(8) * Pmax_TR *...
        (1 - exp(-po_vals(83) * (y_vals(7)/y_vals(8)) * PAR / Pmax_TR)) *...
        exp(-po_vals(79) * PAR);
end
...
growTRn = min(y_vals(8) * po_vals(6) * tfun(T) * Vmax_TR_n,...
...
Pmax_UN = po_vals(37) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
if Pmax_UN == 0
    growUNc = 0;
else
    growUNc = nanmax(y_vals(4) * Pmax_UN *...
        (1 - exp(-po_vals(82) * (y_vals(3)/y_vals(4)) * PAR / Pmax_UN)) *...
        exp(-po_vals(78) * PAR), 0);
end
% N assimilation, including fixation.
...
% total assimilation of N
growUNn = min([y_vals(4) * po_vals(4) * tfun(T) * Vmax_UN_n,...
...
dydtt(21) = fluxBAnh4 + remi_PRT_n + remi_MZ_n + remi_HZ_n + ...
    excr_TR_n_nh4 + excr_UN_n_nh4 -...
    growPHYnh4 - growTRnh4 - growUNnh4 - NTRF;
dydtt(20) = fluxBAno3 - growPHYno3 - growTRno3 - growUNno3 + NTRF;
dydtt(15) = fluxBApo4 + remi_PRT_p + remi_MZ_p + remi_HZ_p -...
