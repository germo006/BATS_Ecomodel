...
function [dydtt] = ode_mod_ecosys_noUN(y, ps, po, ts, z, concat3PAR, concat3PARsig)
...
% Converting Inputs 
%------------------------------------------------------------------------
% Because diff eq. solvers like vectors as inputs, we're going to take the
% input vector and transform it into a matrix for easy indexing. 
%y = reshape(y, length(y)/length(z), length(z));
...
growPHYchl = nanmax([v0,po.theta .* growPHYn .* growPHYc ./ ...
    (po.alpha_PHY_chl .* y.iPHYchl .* PAR .* exp(-po.beta_PHY .* PAR))], [], 2);
...
excr_PHY_c_act = 0.5 .* y.iPHYc .* max([v0,...
    1-(y.iPHYn./(y.iPHYc .* ps.q_PHY_n_rdf)),...
    1-(y.iPHYp./(y.iPHYc .* ps.q_PHY_p_rdf))], [], 2);
...
graz_PHY_c = tfun(T) .* po.mu_PRT .* y.iPRTc .* y.iPHYc.^2 ./ ...
    (y.iPHYc.^2 + po.g_PHY.^2 +...%    (y.iUNc.*po.g_PHY./po.g_UN).^2 +
...
pickTRpo4 = max([v0, po.picktrpo4 .* (y.iTRc .* ((ps.q_TR_p_max + ps.q_TR_p_rdf)./2) - ...
    y.iTRp)], [], 2);
...
condPAR = (PAR ==0);
growTRchl(condPAR) = 0;
growTRchl(~condPAR) = po.theta .* growTRn .* growTRc ./ ...
...
excr_TR_c_act = 0.5 .* y.iTRc .* max([v0,...
    1-(y.iTRn./(y.iTRc .* ps.q_TR_n_rdf)),...
    1-(y.iTRp./(y.iTRc .* ps.q_TR_p_rdf))], [], 2);
...
growBAsdon = growBAsdoc .* min([ps.q_BA_n *v1,...
    y.iSDOMn./y.iSDOMc + po.f_BAslct./Nfunc_ba_n.*(ps.q_BA_n-(y.iSDOMn./y.iSDOMc))],[], 2);
growBAsdop = growBAsdoc .* min([ps.q_BA_p *v1,...
    y.iSDOMp./y.iSDOMc + po.f_BAslct./Nfunc_ba_p.*(ps.q_BA_p-(y.iSDOMp./y.iSDOMc))],[], 2);

% inorganic nutrients uptake
growBAnh4 = (growBAldon .* y.iNH4 .* min([v1, 1./Nfunc_ba_n],[], 2))./ y.iLDOMn;
if Nfunc_ba_n < 1
    growBAno3 = min([0.1 .* ((growBAldon + growBAsdon) ./ (y.iLDOMn + y.iSDOMn)) .* y.iNO3 .* min(v1, 1./Nfunc_ba_n),...
        ((growBAldon + growBAsdon) ./ (y.iLDOMn + y.iSDOMn)) .* (y.iNO3  + y.iNH4) - growBAnh4],[], 2);
    growBAno3 = max([v0, growBAno3],[], 2);
else
    growBAno3 = 0;
end
growBApo4 = (growBAldop ./ y.iLDOMp) .* y.iPO4 .* min([v1, 1./Nfunc_ba_p],[], 2);
...
condC = (y.iBAc < y.iBAn./ps.q_BA_n) & (y.iBAc < y.iBAp./ps.q_BA_p); %Carbon in short
excrBAc(condC) = 0;
excrBAn(condC) = 0;
excrBAp(condC) = 0;
remiBAn(condC) = po.r_BAremi .* (y.iBAn(condC) - y.iBAc(condC) .* ps.q_BA_n);
remiBAp(condC) = po.r_BAremi .* (y.iBAp(condC) - y.iBAc(condC) .* ps.q_BA_p);

condN = (y.iBAc > y.iBAn./ps.q_BA_n) & (y.iBAp./ps.q_BA_p > y.iBAn./ps.q_BA_n); %Nitrogen in short
excrBAc(condN) = po.r_BAadj .* (y.iBAc(condN) - (y.iBAn(condN)./ps.q_BA_n));
excrBAn(condN) = 0;
excrBAp(condN) = po.r_BAadj .* (y.iBAp(condN) - ((y.iBAn(condN)./ps.q_BA_n) .* ps.q_BA_p));
remiBAn(condN) = 0;
remiBAp(condN) = 0;

condP = ~ (condC | condN);
excrBAc(condP) = po.r_BAadj .* (y.iBAc(condP) - (y.iBAp(condP)./ps.q_BA_p));
excrBAn(condP) = po.r_BAadj .* (y.iBAn(condP) - ((y.iBAp(condP)./ps.q_BA_p) .* ps.q_BA_n));
excrBAp(condP) = 0;
remiBAn(condP) = 0;
remiBAp(condP) = 0;
...
grazBAc = po.mu_PRT .* y.iPRTc .* y.iBAc.^2 ./...
    (y.iBAc.^2 + po.g_BA.^2 + ...%     (y.iUNc.*po.g_BA./po.g_UN).^2) +
...
%7. BA Derivs
dydtt.iBAc = (growBAc - refrBAc - excrBAc - grazBAc - respBA - mortBAc); 
...
excr2_PRT_SDOC = po.ex_adj_PRT .* y.iPRTc .* max([v0,...
    1 - (y.iPRTn./(y.iPRTc.*ps.q_PRT_n)),...
    1 - (y.iPRTp./(y.iPRTc.*ps.q_PRT_p))], [], 2);
...
remi_PRT_n = po.remi_PRT .* max([v0, y.iPRTn - (ps.q_PRT_n .* y.iPRTc),...
    y.iPRTn - (ps.q_PRT_n .* y.iPRTp ./ ps.q_PRT_p)], [], 2);
remi_PRT_p = po.remi_PRT .* max([v0, y.iPRTp - (ps.q_PRT_p .* y.iPRTc),...
    y.iPRTp - (ps.q_PRT_p .* y.iPRTn ./ ps.q_PRT_n)], [], 2);
...
excr2_MZ_SDOC = po.ex_adj_MZ .* y.iMZc .* max([v0,...
    1 - (y.iMZn./(y.iMZc.*ps.q_MZ_n)),...
    1 - (y.iMZp./(y.iMZc.*ps.q_MZ_p))], [], 2);
excr2_MZ_SDON = 0.5 .* excr2_MZ_SDOC .* (y.iMZn./y.iMZc);
excr2_MZ_SDOP = 0.5 .* excr2_MZ_SDOC .* (y.iMZp./y.iMZc);

% Inorganic nutrient remin to adjust stoichiometry.

remi_MZ_n = po.remi_MZ .* max([v0, y.iMZn - (ps.q_MZ_n .* y.iMZc),...
    y.iMZn - (ps.q_MZ_n .* y.iMZp ./ ps.q_MZ_p)], [], 2);
remi_MZ_p = po.remi_MZ .* max([v0, y.iMZp - (ps.q_MZ_p .* y.iMZc),...
    y.iMZp - (ps.q_MZ_p .* y.iMZn ./ ps.q_MZ_n)], [], 2);
...
dydtt.iDETc = POM_PHY_c + POM_TR_c + POM_PRT_c + POM_MZ_c + POM_HZ_c - DISS_DET_c;  %+ POM_UN_c
dydtt.iDETn = POM_PHY_n + POM_TR_n + POM_PRT_n + POM_MZ_n + POM_HZ_n - DISS_DET_n;  %+ POM_UN_n
dydtt.iDETp = POM_PHY_p + POM_TR_p + POM_PRT_p + POM_MZ_p + POM_HZ_p - DISS_DET_p; % + POM_UN_p
...
REFR_SDOC = po.ex_REFR_SDOM .* y.iSDOMc .* ...
    ...
    (y.iSDOMp ./ (y.iSDOMc.*po.q_REFR_p))],[], 2) ) + ...
    max([ v0 , y.iSDOMc - (y.iSDOMn./po.q_REFR_n), y.iSDOMc - (y.iSDOMp./po.q_REFR_p)],[], 2);
REFR_SDON = po.q_REFR_n .* REFR_SDOC;
REFR_SDOP = po.q_REFR_p .* REFR_SDOC;

% Change in DOM species

dydtt.iLDOMc = excr_PHY_LDOC + excr_TR_LDOC + excr_PRT_LDOC + ... + excr_UN_LDOC
    excr_MZ_LDOC + mortBAc - growBAldoc;
dydtt.iLDOMn = excr_PHY_LDON + excr_TR_LDON + excr_PRT_LDON + ... + excr_UN_LDON
    excr_MZ_LDON + mortBAn - growBAldon;
dydtt.iLDOMp = excr_PHY_LDOP + excr_TR_LDOP + excr_PRT_LDOP + ... + excr_UN_LDOP
    excr_MZ_LDOP + mortBAp - growBAldop;

dydtt.iSDOMc = excr_PHY_SDOC + excr_TR_SDOC + excrBAc +... + excr_UN_SDOC
    excr_PRT_SDOC + excr2_PRT_SDOC + excr_MZ_SDOC + excr2_MZ_SDOC +...
    excr_HZ_SDOC + DISS_DET_c - REFR_SDOC - growBAsdoc;

dydtt.iSDOMn = excr_PHY_SDON + excr_TR_SDON + excrBAn +... + excr_UN_SDON
    excr_PRT_SDON + excr2_PRT_SDON + excr_MZ_SDON + excr2_MZ_SDON +...
    excr_HZ_SDON + DISS_DET_n - REFR_SDON - growBAsdon;

dydtt.iSDOMp = excr_PHY_SDOP + excr_TR_SDOP + excrBAp +... + excr_UN_SDOP
...
%   
dydtt.pp = sum(diaginput(:,1:8),2);
dydtt.bp = sum(diaginput(:,9:10),2);
