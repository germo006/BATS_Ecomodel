...
% sense)
pickTRpo4 = max([v0, po_vals(33) .* (y_vals(5,:) .* ((ps_vals(2) + ps_vals(1))./2) - ...
    y_vals(2,:))], [], 2);
...
condC = (y_vals(30,:) < y_vals(29,:)./ps_vals(18)) & (y_vals(30,:) < y_vals(28,:)./ps_vals(17)); %Carbon in short
excrBAc(condC) = 0;
excrBAn(condC) = 0;
excrBAp(condC) = 0;
remiBAn(condC) = po_vals(19) .* (y_vals(29,condC) - y_vals(30,condC) .* ps_vals(18));
remiBAp(condC) = po_vals(19) .* (y_vals(28,condC) - y_vals(30,condC) .* ps_vals(17));
condN = (y_vals(30,:) > y_vals(29,:)./ps_vals(18)) & (y_vals(28,:)./ps_vals(17) > y_vals(29,:)./ps_vals(18)); %Nitrogen in short
excrBAc(condN) = po_vals(22) .* (y_vals(30,condN) - (y_vals(29,condN)./ps_vals(18)));
excrBAn(condN) = 0;
excrBAp(condN) = po_vals(22) .* (y_vals(28,condN) - ((y_vals(29,condN)./ps_vals(18)) .* ps_vals(17)));
remiBAn(condN) = 0;
remiBAp(condN) = 0;
condP = ~ (condC | condN);
excrBAc(condP) = po_vals(22) .* (y_vals(30,condP) - (y_vals(28,condP)./ps_vals(17)));
excrBAn(condP) = po_vals(22) .* (y_vals(29,condP) - ((y_vals(28,condP)./ps_vals(17)) .* ps_vals(18)));
excrBAp(condP) = 0;
remiBAn(condP) = 0;
remiBAp(condP) = 0;
% excrBAc = excrBAc.';
...
excr2_MZ_SDOC = po_vals(57) .* y_vals(21,:) .* max([v0,...
    1 - (y_vals(20,:)./(y_vals(21,:).*ps_vals(16))),...
    1 - (y_vals(19,:)./(y_vals(21,:).*ps_vals(15)))], [], 2);
excr2_MZ_SDON = 0.5 .* excr2_MZ_SDOC .* (y_vals(20,:)./y_vals(21,:));
excr2_MZ_SDOP = 0.5 .* excr2_MZ_SDOC .* (y_vals(19,:)./y_vals(21,:));
% Inorganic nutrient remin to adjust stoichiometry.
remi_MZ_n = po_vals(14) .* max([v0, y_vals(20,:) - (ps_vals(16) .* y_vals(21,:)),...
    y_vals(20,:) - (ps_vals(16) .* y_vals(19,:) ./ ps_vals(15))], [], 2);
remi_MZ_p = po_vals(14) .* max([v0, y_vals(19,:) - (ps_vals(15) .* y_vals(21,:)),...
    y_vals(19,:) - (ps_vals(15) .* y_vals(20,:) ./ ps_vals(16))], [], 2);
% POM production
