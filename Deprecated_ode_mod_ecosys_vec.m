...
%-----------------------------------------------------------------------
% 1. Gross Grow
% Deternanmining growth rate limits
...
temp = nanmax([nanmin([Nfunc_phy_n, Nfunc_phy_p, 1]),0]); % Force elemental limits
% on rate to between 0 and 100%
Pnanmax_phy = po_vals(40) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
growPHYc = y_vals(19) * Pnanmax_phy *...
    (1 - exp(-po_vals(84) * (y_vals(18)/y_vals(19)) * PAR / Pnanmax_phy)) *...
    exp(-po_vals(80) * PAR);
% Nitrogen assimilation
Vnanmax_phy_n = (ps_vals(19) - (y_vals(17)/y_vals(19)))/(ps_vals(19) - ps_vals(18));
temp = nanmax([nanmin([Vnanmax_phy_n,1]),0]); % Yet again forcing this.
growPHYnh4 = y_vals(19) * po_vals(8) * tfun(T) * temp * ...
    y_vals(21) / (y_vals(21) + po_vals(51) + (y_vals(20)*po_vals(51)/po_vals(50)));
growPHYno3 = y_vals(19) * po_vals(8) * tfun(T) * temp * ...
...
Vnanmax_phy_p = (ps_vals(16) - (y_vals(16)/y_vals(19)))/(ps_vals(16) - ps_vals(15));
temp = nanmax([nanmin([Vnanmax_phy_p,1]),0]); % Yet again forcing this.
% Assunanming inorganic P is the limiter and supply, here's the nanmax growth.
growPHYpo4 = y_vals(19) * po_vals(7) * tfun(T) * temp * ...
...
growPHYchl = po_vals(9) * growPHYn * growPHYc / ...
    (po_vals(84) * y_vals(18) * PAR * exp(-po_vals(80) * PAR));
% DOM excretion!
...
excr_PHY_c_act = 0.5 * y_vals(19) * nanmax([0,...
    1-(y_vals(17)/(y_vals(19) * ps_vals(18))),...
    1-(y_vals(16)/(y_vals(19) * ps_vals(15)))]);
if excr_PHY_c_act > 0
    excr_PHY_n_act = 0.5 * 0.25 * y_vals(17) * nanmax([0,...
        1 - (y_vals(16) / y_vals(17))/(ps_vals(15) / ps_vals(18))]);
    
    excr_PHY_p_act = 0.5 * 0.25 * y_vals(16) * nanmax([0,...
...
% Grazing
graz_PHY_c = tfun(T) * po_vals(39) * y_vals(14) * y_vals(19)^2 / ...
...
temp = nanmax([nanmin([Nfunc_TR_n, Nfunc_TR_p, 1]),0]);
Pnanmax_TR = po_vals(38) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
if Pnanmax_TR == 0
    growTRc = 0;
else
    growTRc = y_vals(8) * Pnanmax_TR *...
        (1 - exp(-po_vals(83) * (y_vals(7)/y_vals(8)) * PAR / Pnanmax_TR)) *...
        exp(-po_vals(79) * PAR);
end
% N assimilation, including fixation.
Vnanmax_TR_n = (ps_vals(11) - (y_vals(6)/y_vals(8)))/(ps_vals(11) - ps_vals(10));
temp = nanmax(nanmin(Vnanmax_TR_n,1),0); % Yet again forcing this.
growTRnh4 = y_vals(8) * po_vals(6) * tfun(T) * temp * ...
    y_vals(21) / (y_vals(21) + po_vals(48) + (y_vals(20)*po_vals(48)/po_vals(47)));
growTRno3 = y_vals(8) * po_vals(6) * tfun(T) * temp * ...
    y_vals(20) / (y_vals(20) + po_vals(47) + (y_vals(21)*po_vals(47)/po_vals(48)));
% Fixation
if T <= 20
    nfixTRnanmax = 0;
else
    nfixTRnanmax = tfun(T) * Vnanmax_TR_n * ...
        (((growTRc - (po_vals(1)*growTRno3)) * ps_vals(11)) - ...
        growTRno3 - growTRnh4)/...
        (1 + (po_vals(2) * ps_vals(11)));
    nfixTRnanmax = nanmax([0, nfixTRnanmax]);
end
% total assimilation of N
growTRn = nanmin(y_vals(8) * po_vals(6) * tfun(T) * Vnanmax_TR_n,...
    growTRnh4 + growTRno3 + nfixTRnanmax);
% Actual (not nanmax) fixation rate
growTRnfix = growTRn - growTRnh4 - growTRno3;
% Phosphorus assimilation
Vnanmax_TR_p = (ps_vals(8) - (y_vals(5)/y_vals(8)))/(ps_vals(8) - ps_vals(7));
temp = nanmax([nanmin([Vnanmax_TR_p,1]),0]); % Yet again forcing this.
% Assunanming inorganic P is the limiter and supply, here's the nanmax growth.
growTRpo4 = y_vals(8) * po_vals(5) * tfun(T) * temp * ...
    y_vals(15) / (y_vals(15) + po_vals(46));
% Picking up PO4 from deep water (kind of unclear how this term makes
% sense)
pickTRpo4 = nanmax([0, po_vals(36) * (y_vals(8) * ((ps_vals(8) + ps_vals(7))/2) - ...
    y_vals(5))]);
...
if PAR==0
    ...
    growTRchl = po_vals(9) * growTRn * growTRc / ...
        (po_vals(83) * y_vals(7) * PAR * exp(-po_vals(79) * PAR));
end
% DOM excretion!
% Passive (fixed proportion)
...
% Active excretion to adjust stoichiometry.
excr_TR_c_act = 0.5 * y_vals(8) * nanmax([0,...
    1-(y_vals(6)/(y_vals(8) * ps_vals(10))),...
    1-(y_vals(5)/(y_vals(8) * ps_vals(7)))]);
if excr_TR_c_act > 0
    excr_TR_n_act = 0.5 * 0.25 * y_vals(6) * nanmax([0,...
        1 - (y_vals(5) / y_vals(6))/(ps_vals(7) / ps_vals(10))]);
    
    excr_TR_p_act = 0.5 * 0.25 * y_vals(5) * nanmax([0,...
...
% Grazing
graz_TR_c = tfun(T) * po_vals(41) * y_vals(24) * y_vals(8)^2 / ...
...
temp = nanmax([nanmin([Nfunc_UN_n, Nfunc_UN_p, 1]),0]);
Pnanmax_UN = po_vals(37) * tfun(T) * temp; % Max specific growth rate.
% Light limitation and specific gross PP (as carbon)
if Pnanmax_UN == 0
    growUNc = 0;
else
    growUNc = y_vals(4) * Pnanmax_UN *...
        (1 - exp(-po_vals(82) * (y_vals(3)/y_vals(4)) * PAR / Pnanmax_UN)) *...
        exp(-po_vals(78) * PAR);
end
% N assimilation, including fixation.
Vnanmax_UN_n = (ps_vals(5) - (y_vals(2)/y_vals(4)))/(ps_vals(5) - ps_vals(4));
temp = nanmax([nanmin([Vnanmax_UN_n,1]),0]); % Yet again forcing this.
growUNnh4 = y_vals(4) * po_vals(4) * tfun(T) * temp * ...
    y_vals(21) / (y_vals(21) + po_vals(45) + (y_vals(20)*po_vals(45)/po_vals(44)));
growUNno3 = y_vals(4) * po_vals(4) * tfun(T) * temp * ...
    y_vals(20) / (y_vals(20) + po_vals(44) + (y_vals(21)*po_vals(44)/po_vals(45)));
% Fixation
if T <= 15
    nfixUNnanmax = 0;
else
    nfixUNnanmax = tfun(T) * Vnanmax_UN_n * ...
        (((growUNc - (po_vals(1)*growUNno3)) * ps_vals(5)) - ...
        growUNno3 - growUNnh4)/...
        (1 + (po_vals(2) * ps_vals(5)));
    nfixUNnanmax = nanmax([0, nfixUNnanmax]);
end
% total assimilation of N
growUNn = nanmin([y_vals(4) * po_vals(4) * tfun(T) * Vnanmax_UN_n,...
    growUNnh4 + growUNno3 + nfixUNnanmax]);
% Actual (not nanmax) fixation rate
growUNnfix = growUNn - growUNnh4 - growUNno3;
% Phosphorus assimilation
Vnanmax_UN_p = (ps_vals(2) - (y_vals(1)/y_vals(4)))/(ps_vals(2) - ps_vals(1));
temp = nanmax([nanmin([Vnanmax_UN_p,1]),0]); % Yet again forcing this.
% Assunanming inorganic P is the limiter and supply, here's the nanmax growth.
growUNp = y_vals(4) * po_vals(3) * tfun(T) * temp * ...
...
if PAR == 0
    ...
    growUNchl = po_vals(9) * growUNn * growUNc / ...
        (po_vals(82) * y_vals(3) * PAR * exp(-po_vals(78) * PAR));
end
% DOM excretion!
% Passive (fixed proportion)
...
% Active excretion to adjust stoichiometry.
excr_UN_c_act = 0.5 * y_vals(4) * nanmax([0,...
    1-(y_vals(2)/(y_vals(4) * ps_vals(4))),...
    1-(y_vals(1)/(y_vals(4) * ps_vals(1)))]);
if excr_UN_c_act > 0
    excr_UN_n_act = 0.5 * 0.25 * y_vals(2) * nanmax([0,...
        1 - (y_vals(1) / y_vals(2))/(ps_vals(1) / ps_vals(4))]);
    
    excr_UN_p_act = 0.5 * 0.25 * y_vals(1) * nanmax([0,...
...
% Grazing
graz_UN_c = tfun(T) * po_vals(39) * y_vals(14) * y_vals(4)^2 / ...
...
temp = nanmin([Nfunc_ba_n, Nfunc_ba_p]);
temp = nanmin(temp, 1); % Make sure the limit is unity (not the case for mtabs?)
growBAldoc = po_vals(42) * y_vals(33) * tfun(T) * temp * ALC / (ALC+ po_vals(52) + ASC);
growBAsdoc = po_vals(42) * y_vals(33) * tfun(T) * temp * ASC / (ASC+ po_vals(52) + ALC);
...
growBAsdon = growBAsdoc * nanmin([ps_vals(24),...
    y_vals(10)/y_vals(11) + po_vals(62)/Nfunc_ba_n*(ps_vals(24)-(y_vals(10)/y_vals(11)))]);
growBAsdop = growBAsdoc * nanmin([ps_vals(23),...
    y_vals(9)/y_vals(11) + po_vals(62)/Nfunc_ba_p*(ps_vals(23)-(y_vals(9)/y_vals(11)))]);
% inorganic nutrients uptake
growBAnh4 = (growBAldon * y_vals(21) * nanmin(1, 1/Nfunc_ba_n))/ y_vals(26);
if Nfunc_ba_n < 1
    growBAno3 = nanmin([0.1 * ((growBAldon + growBAsdon) / (y_vals(26) + y_vals(10))) * y_vals(20) * nanmin(1, 1/Nfunc_ba_n),...
        ((growBAldon + growBAsdon) / (y_vals(26) + y_vals(10))) * (y_vals(20)  + y_vals(21)) - growBAnh4]);
    growBAno3 = nanmax([0, growBAno3]);
else
    growBAno3 = 0;
end
growBApo4 = (growBAldop / y_vals(25)) * y_vals(15) * nanmin([1, 1/Nfunc_ba_p]);
...
% Respiration
resp_PRT = (po_vals(10) * tfun(T) * y_vals(14)) + (po_vals(12) * growPRTc);
% SDOM release to adjust stoichiometry
excr2_PRT_SDOC = po_vals(64) * y_vals(14) * nanmax([0,...
...
% Inorganic nutrient renanmin to adjust stoichiometry.
remi_PRT_n = po_vals(15) * nanmax([0, y_vals(13) - (ps_vals(14) * y_vals(14)),...
    y_vals(13) - (ps_vals(14) * y_vals(12) / ps_vals(13))]);
remi_PRT_p = po_vals(15) * nanmax([0, y_vals(12) - (ps_vals(13) * y_vals(14)),...
...
graz_PRT_c = tfun(T) * po_vals(41) * y_vals(24) * y_vals(14)^2 / ...
...
% Respiration
resp_MZ = (po_vals(11) * tfun(T) * y_vals(24)) + (po_vals(13) * growMZc);
% Semilabile DOM stoich adjustment
excr2_MZ_SDOC = po_vals(65) * y_vals(24) * nanmax([0,...
...
% Inorganic nutrient renanmin to adjust stoichiometry.
remi_MZ_n = po_vals(16) * nanmax([0, y_vals(23) - (ps_vals(22) * y_vals(24)),...
    y_vals(23) - (ps_vals(22) * y_vals(22) / ps_vals(21))]);
remi_MZ_p = po_vals(16) * nanmax([0, y_vals(22) - (ps_vals(21) * y_vals(24)),...
...
% Inorganic nuts renanmineralized by HZ
...
REFR_SDOC = po_vals(72) * y_vals(11) * ...
    exp( 1 - nanmin([(y_vals(10) / (y_vals(11)*po_vals(26))),...
    (y_vals(9) / (y_vals(11)*po_vals(25)))]) ) + ...
    nanmax([ 0 , y_vals(11) - (y_vals(10)/po_vals(26)), y_vals(11) - (y_vals(9)/po_vals(25))]);
